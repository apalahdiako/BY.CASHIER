...
type PendingAuthMode = "register" | "register-verify" | "forgot-code" | "forgot-pin";
...
export function AccountLogin({ close }: { close: () => void }) {
  ...
  const [mode, setMode] = useState<"login" | "register" | "register-verify" | "forgot-code" | "forgot-pin">(() => readPendingAuth()?.mode || "login");
  ...
  const finishSignUp = async (attempt: { status: string | null; createdSessionId: string | null }) => { if (attempt.status !== "complete" || !attempt.createdSessionId || !setActiveSignUp) throw new Error("Verifikasi akun belum selesai."); await setActiveSignUp({ session: attempt.createdSessionId }); closeLogin(); };
  const submit = async () => {
    ...
    try {
      if (mode === "login") {
        ...
        const attempt = await signIn.create({ identifier: id.trim(), password: pin });
        if (attempt.status !== "complete" || !attempt.createdSessionId) throw new Error("Login membutuhkan verifikasi tambahan pada akun ini.");
        await setActiveSignIn({ session: attempt.createdSessionId }); closeLogin();
      } else {
        if (!signUpReady || !signUp) return;
        const attempt = await signUp.create({ username: id.trim(), emailAddress: email.trim(), password: pin });
  ...
  const verifyRegistration = async () => {
    ...
    setLoading(true); setError("");
    try { await finishSignUp(await signUp.attemptEmailAddressVerification({ code })); } catch (reason) { setError(clerkError(reason)); } finally { setLoading(false); }
  };
  const resendRegistrationCode = async () => {
    if (!signUp) return;
    setLoading(true); setError("");
    try { await signUp.prepareEmailAddressVerification({ strategy: "email_code" }); } catch (reason) { setError(clerkError(reason)); } finally { setLoading(false); }
  };
  const startReset = async () => {
    if (!id.trim() || !signInReady || !signIn) return;
    setLoading(true); setError("");
    try {
      const attempt = await signIn.create({ identifier: id.trim() });
  ...
  const saveNewPin = async () => {
    ...
    setLoading(true); setError("");
    try { const attempt = await signIn.resetPassword({ password: pin, signOutOfOtherSessions: true }); if (attempt.status !== "complete" || !attempt.createdSessionId) throw new Error("PIN baru belum dapat disimpan."); await setActiveSignIn({ session: attempt.createdSessionId }); closeLogin(); } catch (reason) { setError(clerkError(reason)); } finally { setLoading(false); }
  };
  const isFlow = mode === "register-verify" || mode === "forgot-code" || mode === "forgot-pin";
  useEffect(() => { if (isFlow) window.localStorage.setItem(pendingAuthKey, JSON.stringify({ mode, id, email })); }, [email, id, isFlow, mode]);
  const isCodeStep = mode === "register-verify" || mode === "forgot-code";
  const title = mode === "login" ? "Masuk ke akun" : mode === "register" ? "Daftarkan akun" : mode === "register-verify" ? "Verifikasi Gmail" : mode === "forgot-code" ? "Cek email Gmail" : "Buat PIN baru";
  const description = mode === "login" ? "Masuk dengan ID akun atau Gmail dan PIN yang terdaftar." : mode === "register" ? "Buat ID, Gmail, dan PIN untuk akun baru." : mode === "register-verify" ? "Masukkan kode yang Clerk kirim ke Gmail kamu agar akun aktif." : mode === "forgot-code" ? "Masukkan kode reset yang dikirim ke Gmail akun ini." : "Masukkan PIN unik berisi tepat 8 digit.";
  const disabled = loading || !id.trim() || (mode === "login" ? !pin : mode === "register" ? !email.trim() || pin.length !== 8 : isCodeStep ? !code : pin.length !== 8);
  const action = mode === "login" || mode === "register" ? submit : mode === "register-verify" ? verifyRegistration : mode === "forgot-code" ? verifyResetCode : saveNewPin;
